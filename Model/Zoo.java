...
public class Zoo<E> implements Serializable {
	...
	public boolean newVisitorInZoo(Visitor v, Section s)  {
		...
		try 
		{
			...
			// adding visitor and money to the revenue
	...
	//this method gets a section and returns all the animals in it sorted 
	public ArrayList<Animal> getAllAnimalsBySectionMaxVisits(Section s) {
		...
		animalsList.sort(new Comparator<Animal>() {
			@Override
			public int compare(Animal o1, Animal o2) 
			{
				String theName1 = o1.getClass().getSimpleName();
				String theName2 = o2.getClass().getSimpleName();
				if(theName1.equals(theName2)) 
				{
					Integer visits1 = o1.getVisitCounter();
					Integer visits2 = o2.getVisitCounter();
					return visits1.compareTo(visits2);
				}
				else
	...
	//this method returns animals that the worker did not treat, by specific requirements
	@SuppressWarnings("null")
	public ArrayList<Animal> allAnimalsByWorker(ZooEmployee employee){
		ArrayList<Animal> treatedList = new ArrayList<Animal>();
		if(getEmployees().containsValue(employee)) 
		{
	...
	public ArrayList<Snack>  findAllSnackByWorker(SnackBar sb){
		...
		snacksList.sort(new Comparator<Snack>(){
			@Override
	...
	//this method returns a list of all reptiles who sleep and are not dangerous
	public ArrayList<Reptile> reptilesSleepAtSeasson()
	{
		...
		reptilesList.sort(new Comparator<Reptile>() 
		{
			@Override
	...
	public TreeMap<Visitor, Double> geAllDiscountAmount()
	{
		...
		for(Visitor v : getVisitors().values()) 
		{
			...
			if(discountPrice > 0.0) // because if not we show all the visitors with no discount
				discountTree.put(v, discountPrice);
		}
		return discountTree;
	}

	//this method calculate the maximal difference between visitors size and employee size in a section
	public Section getMaxVisitorsVSMaxWorkers() 
	{
		PriorityQueue<Section> biggestDifference = new PriorityQueue<Section>(new Comparator<Section>() {
			@Override
			public int compare(Section o1, Section o2) 
			{
				int size1= o1.getVisitors().size() - o1.getEmployees().size();
				int size2 = o2.getVisitors().size() - o2.getEmployees().size();
				return size2 - size1;
			}
		});
		...
		for(Section s: getSections().values()) 
		{
			biggestDifference.add(s);
		}
		return biggestDifference.poll();
	}
	...
	{
		String str ="";
		ArrayList<String>arr = new ArrayList<String>(); //here I will save the casting of the array to string
		for(Object ob: brr)
		{
			arr.add(ob.toString()); //casting from object to string
		}
		
		ArrayList<String> fieldNames = new ArrayList<String>(); //here I will save the strings to return
		for(String s: arr) 
		{
			...
			fieldNames.add(str); //adding the field name to the array I will return
		}
		fieldNames.remove("id"); //removing fields I do not want the user to see
