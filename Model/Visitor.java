...
public class Visitor extends Person implements Serializable 
{
	...
	{
		try 
		{ //need to check if we are in the 50% capacity, because of Corona capsules
	...
	{	//checking if drink or food, because each one has different prices check, by it's attributes
		Drink tmp = (Drink)s;
		...
		if(isValid && tmp.isPlate() && this.getSection().getBar().getSnacks().contains(tmp)){
			...
			String str ="Visitor: "+this.getFirstName()+" Purchased Food: "+tmp.getSnackName() +" Price: " + tmp.getPrice();
			alertSucssedMessage(str);
			saveCounterForBuy(s); //keeping track of what is bought, for profit purpose
			return true;
		}
		else {
